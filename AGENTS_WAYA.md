...
> 以下は `docker` ブランチにのみ存在する機能です。`upstream` / `main` への逆流は禁止されています。  
> 詳細は `README_DOCKER.md` を参照。

### ytdlp-core-api 連携（IP バイパス）

`YTDLP_API_URL` 環境変数が設定されている場合、ダウンロード・プレイリスト取得を  
WARP ルーティング付きの `ytdlp-core-api` コンテナ経由で行います。

| ファイル | 概要 |
|----------|------|
| `server/ytdlp_api_service.py` | API 経由ダウンロードサービス（新規） |
| `server/ytdlp_service.py` | `YTDLP_API_URL` 設定時のルーティング追加 |
| `server/sync_service.py` | プレイリスト同期の API ルーティング追加 |
| `docker-compose.yml` | `YTDLP_API_URL=http://ytdlp-core-api:8000` + `ytdlp-network` |

**設計ポイント:**
- `YTDLP_API_URL` が未設定なら従来の直接 yt-dlp 呼び出しにフォールバック
- `ytdlp-core-api` は WARP (Cloudflare) 経由で YouTube などにアクセスするため IP 制限を回避できる
- `ytdlp_api_service.py` の関数は直接呼び出さず、必ず `ytdlp_service.py` / `sync_service.py` 経由で使用する

### 追加 Python 依存（docker ブランチのみ）

| パッケージ | 用途 |
|-----------|------|
| `Pillow` | 画像処理 |
| `requests` | ytdlp-core-api HTTP 呼び出し |
| `redis` | Redis クライアント（将来的な拡張用） |
| `certifi` | SSL 証明書（ARM 環境向け） |
